...
(function () {
  ...
  /* ---- Traço de caneta sobre o papel da hero -------------------------- */

  /* O traço nasce só onde há papel: se o ponteiro está sobre o título, um
     cartão ou o retrato, nada é desenhado. A decisão é por lista branca de
     contêineres, não por lista negra de conteúdo — conteúdo novo entra sem
     precisar ser lembrado aqui. */
  var hero = document.querySelector(".hero");
  var inkCanvas = document.querySelector("[data-ink]");

  if (hero && inkCanvas && inkCanvas.getContext && !reduced.matches) {
    ...
    var cssW = 0;
    var cssH = 0;
    var LIFE = 900; // ms de vida de cada segmento

    var PAPER = ["hero", "hero__inner", "hero__lede", "hero__stage", "hero__thread"];

    function isPaper(el) {
      if (!el || !el.classList) return false;
      for (var i = 0; i < PAPER.length; i++) {
        if (el.classList.contains(PAPER[i])) return true;
      }
      return false;
    }

    /* O bitmap tem de acompanhar a caixa da tela. Se a hero cresce depois —
       fontes que chegam, retrato que carrega, janela que muda — e o bitmap
       fica com o tamanho velho, o navegador estica o desenho e o traço passa
       a cair longe do cursor. Por isso a medida é do próprio canvas, não da
       hero, e é conferida antes de cada risco. */
    var inkDpr = 0;
    ...
    function sizeInk() {
      ...
      if (r.width === cssW && r.height === cssH && dpr === inkDpr) return false;
    ...
    function bandOf(m, now) {
      var b = Math.floor(((now - m.t) / LIFE) * BANDS);
    ...
    function inkFrame(now) {
      ...
      for (var i = 0; i < marks.length; i++) {
        if (now - marks[i].t < LIFE) marks[alive++] = marks[i];
      }
      marks.length = alive;

      /* Cada segmento tem uma idade própria, mas traçá-los um a um faz as
         pontas arredondadas se sobreporem e o risco vira um tracejado de
         contas escuras. Então os segmentos são agrupados em poucas faixas de
         idade — dentro de uma faixa é uma linha só, contínua. */
      var start = 0;
      while (start < alive) {
        var band = bandOf(marks[start], now);
        var end = start;
        var wsum = 0;

        while (end < alive && bandOf(marks[end], now) === band) {
          wsum += marks[end].w;
          end++;
        }

        // Some devagar no começo e rápido no fim, como tinta secando.
        var k = (band + 0.5) / BANDS;
        var alpha = (1 - k * k) * 0.7;

        ictx.strokeStyle = "rgba(11, 21, 36, " + alpha.toFixed(3) + ")";
        ictx.lineWidth = wsum / (end - start);
        ictx.beginPath();
        ictx.moveTo(marks[start].x0, marks[start].y0);
        for (var q = start; q < end; q++) {
          ictx.quadraticCurveTo(marks[q].cx, marks[q].cy, marks[q].x1, marks[q].y1);
        }
        ictx.stroke();
      ...
      if (alive > 0) {
        window.requestAnimationFrame(inkFrame);
      } else {
        inkRunning = false;
      }
    }
    ...
    hero.addEventListener("pointermove", function (e) {
      ...
      // A caixa mudou desde a última medida: remede antes de riscar torto.
      ...
      if (lastPoint) {
        ...
        // Salto grande significa que o ponteiro reapareceu em outro lugar:
        // não se liga um ponto ao outro com um risco atravessando a tela.
        if (d > 0.6 && d < 160) {
          // Caneta de verdade: quanto mais rápido o traço, mais fino o risco.
          marks.push({
            x0: lastPoint.x,
            y0: lastPoint.y,
            cx: lastPoint.x + dx * 0.5,
            cy: lastPoint.y + dy * 0.5,
            x1: x,
            y1: y,
            w: Math.max(0.75, 2.5 - d * 0.055),
    ...
    hero.addEventListener("pointerleave", function () { lastPoint = null; });

    sizeInk();
    window.addEventListener("resize", sizeInk, { passive: true });

    // A hero muda de altura depois do carregamento mais de uma vez: quando a
    // fonte real substitui a de fallback e quando o retrato ocupa seu espaço.
    if (window.ResizeObserver) {
      new ResizeObserver(sizeInk).observe(inkCanvas);
    } else {
      window.addEventListener("load", sizeInk);
      if (document.fonts && document.fonts.ready) document.fonts.ready.then(sizeInk);
    }
  }
